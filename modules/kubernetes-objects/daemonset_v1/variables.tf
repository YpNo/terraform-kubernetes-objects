variable "daemon_sets" {
  description = "A list of Kubernetes DaemonSet configurations."
  type = list(object({
    name        = string
    namespace   = string
    ...
    strategy = optional(object({
      ...
      rolling_update = optional(object({ max_unavailable = optional(string) })) # absolute number or percentage
    }))
  ...
  validation {
    condition = alltrue([
      for dep_item in var.daemon_sets :
      alltrue([
  ...
  validation {
    condition = alltrue([
      for item in var.daemon_sets :
      item.strategy == null || item.strategy.type == null || contains(["RollingUpdate", "OnDelete"], item.strategy.type)
    ])
    error_message = "Invalid 'strategy.type'. Must be 'RollingUpdate' or 'OnDelete'."
  }
}
