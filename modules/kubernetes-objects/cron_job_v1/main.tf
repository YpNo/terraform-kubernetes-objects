resource "kubernetes_cron_job_v1" "this" {
  for_each = { for cj in var.cron_jobs : cj.name => cj }
  ...
  spec {
    ...
    job_template {
      metadata {
        labels      = each.value.job_labels
        annotations = each.value.job_annotations
      }
      spec {
        ...
        completion_mode            = each.value.completion_mode
        manual_selector            = each.value.manual_selector
