variable "jobs" {
  description = "A list of Kubernetes Job configurations."
  type = list(object({
    name        = string
    namespace   = string
    ...
    wait_for_completion        = optional(bool) # Whether terraform apply waits for the Job to finish
  ...
  validation {
    ...
    error_message = "Invalid 'restart_policy' for a Job pod. Must be 'OnFailure' or 'Never'."
  }

  validation {
    condition = alltrue([
      for item in var.jobs :
      item.completion_mode == null || contains(["NonIndexed", "Indexed"], item.completion_mode)
    ])
    error_message = "Invalid 'completion_mode'. Must be 'NonIndexed' or 'Indexed'."
  }
}
